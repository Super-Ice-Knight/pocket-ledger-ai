...
当前本地基线：29 项后端测试、5 项 Playwright 测试、前端生产构建全部通过。

2026-07-13 本地 Agent 验收：29 项 pytest、5 项 Playwright 与 Vite 生产构建通过；`1440×900` 与 `390×844` 六个页面无横向溢出；跨月周 `2026/6/29 - 7/5` 的周期统计与日期范围明细一致；交易时间可编辑并按北京时间保存；快速切月和切周都只允许最后一次请求更新页面。线上检查仍以本表“线上行为”部分为准。
...
- [ ] 冷启动期间显示“连接时间比平时稍长”，不会闪现零统计
- [ ] `/api/stats/weekly` 按周一到周日返回统计，跨月周不截断
...
- [ ] 输入 `0`、`0.00` 时显示明确错误且确认按钮禁用
- [ ] 输入超过 `99,999,999.99` 元时前后端均拒绝
- [ ] “7月11日买3杯咖啡花50元”识别为 5000 分
- [ ] 缺金额输入进入 `missing_fields`
- [ ] AI 推断的交易时间在确认表单中可见且可修改
- [ ] 编辑流水可以修改交易时间；跨日、跨周或跨月后统计随之改变
- [ ] 修改已解析的原始描述后显示“待重新解析”，旧草稿不能提交
- [ ] 编辑模式可取消，取消不产生数据库写入
- [ ] 同时包含收入和支出的句子拆成两笔输入，不在一笔草稿中抵消
- [ ] 确认入账后流水更新
- [ ] 流水可切换按月/按周，周期收入、支出、净额和笔数与明细一致
- [ ] 删除前出现确认对话框，取消后数据不变
- [ ] 图表旁存在分类、账户、日均和预算文字
- [ ] 分析页账户图例名称、比例和金额不发生单字断行

## 浏览器尺寸

- [ ] `1440×900`：六个页面布局完整
- [ ] `1280×720` 长设置页：侧栏底板与主工作区等高，滚动到底部时导航仍在视口内
- [ ] `1024px`：桌面侧栏改为上下布局，表单和测试结果无挤压
- [ ] `390×844`：无横向溢出
- [ ] 手机首屏可看到快记输入框和解析按钮
- [ ] 手机快记标题完整显示“AI 解析语义，手动确认入账”，不出现单字断行
...
- [ ] DEFENSE_NOTES 能解释金额、AI 点评缓存、SQLite、设置安全和冷启动
- [ ] API_SPEC 包含月/周流水、点评 GET/POST 契约、`fresh/stale/missing`、只读设置字段与 403 行为
- [ ] DEMO_SCRIPT 可在三分钟内完成
- [ ] 文档中的链接均为真实地址，无过期占位内容
