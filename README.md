...
- 北京时间账本：交易时间统一归一为 `Asia/Shanghai` 的 `+08:00` ISO 时间，月、周和日期分组按业务日期计算。
...
当前公开部署使用 Groq `qwen/qwen3.6-27b`，请求超时为 10 秒。2026-07-13 验收时，Provider 服务端测试为 917 ms，普通中文快记为 3.64 秒，汉字金额“两千元”为 4.87 秒，结构化月度点评为 5.25 秒。线上暂不配置未经验证的备用接口，避免失败链路串行累加等待；Provider 失败日志只记录槽位、模型、耗时、状态与异常类型，不记录 Key 或完整账单原文。
...
`SEED_DEMO_DATA=true` 只用于首次体验或公开演示。种子记录只会在空数据库首次初始化时写入一次；用户删除全部流水后不会在重启时自动恢复。个人账本建议保持 `false`。
...
当前覆盖 29 项后端行为，包括金额上下界、北京时间与 UTC 跨日、编辑后跨周/跨月统计、演示数据只初始化一次、月份参数校验、AI 缺失字段重算、安全日志、金额精度、主备切换、双模型失败兜底、设置只读和 Key 防泄漏。
...
当前 5 项 Playwright 测试覆盖金额与时间转换、旧 AI 草稿失效、交易时间修改后提交、取消编辑、月度请求乱序，以及 `1440×900` / `390×844` 六页面溢出检查。
