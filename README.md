...
- **新建账单**：`AI 快记` → 输入一笔收入或支出 → `解析账单` → 核对金额、分类、账户和交易时间 → `确认入账`。
...
| 任务书关注点 | 当前实现 | 验收证据 |
|---|---|---|
| 金额、分类、账户、时间、备注 | 共用确认/编辑表单，交易时间可修改 | 35 项 pytest、7 项 Playwright |
...
当前 7 项 Playwright 测试覆盖金额与时间转换、旧 AI 草稿失效、交易时间修改后提交、取消编辑、月度请求乱序、显式确认“其他”分类、补齐冲突收支与时间，以及 `1440×900` / `390×844` 六页面溢出检查。
